...
def parse_transcription(wav_path, lang="en"):
    global model_dict, model, generator, target_dict
    try:
        cur_dict = model_dict[lang]
    except:
...
def start_all(model_base_path):
    ...
    for language in languages:
        if language == 'en' and os.path.isdir(os.path.join(model_base_path, language)):
            cur_dict = {}
            lexicon_path = os.path.join(model_base_path, language, "lexicon.lst")
            lm_path = os.path.join(model_base_path, language, "lm.binary")
            dict_path = os.path.join(model_base_path, language, "dict.txt")
            model_path = os.path.join(model_base_path, language, "infer.pt")

            args = get_args(lexicon_path, lm_path)
            cur_dict["target_dict"] = Dictionary.load(dict_path)
            cur_dict["generator"] = W2lKenLMDecoder(args, cur_dict["target_dict"])
            cur_dict["model"] = load_model(model_path)
            model_dict[language] = cur_dict
            print("model loaded" + language)
